...
if ((resetDayLeft !== undefined && resetDayLeft !== null) || expire) { // 如果 resetDayLeft 存在或 expire 存在
  if (resetDayLeft === 0) { // 如果 resetDayLeft 等于 0
    ...
    content.push(`重置：${resetDayLeft}天 \t|  ${formatTime(expire)}`); // 添加格式化后的字符串到 content 数组中
  } else if (resetDayLeft && !expire) { // 如果只有 resetDayLeft 存在
    content.push(`重置：${resetDayLeft}天`); // 添加格式化后的字符串到 content 数组中
  } else if (!resetDayLeft && expire) { // 如果只有 expire 存在
    if (/^[\d.]+$/.test(expire)) expire *= 1000; // 如果 expire 是数字，则将其转换为毫秒数
